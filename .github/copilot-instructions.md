...
* Make changes
* Build the solution using `go build -C src`. This'll typically run a lot faster for catching errors than running a full publish.
* Run `./scripts/publish.ps1 ./bicep-ext-http` to publish the self-contained extension to `./bicep-ext-http`.
